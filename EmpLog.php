...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // $sql = "Select * from empreg where compEmail='$email'";
    $sql = "Select * from empreg where compEmail='$email'";
    $result = mysqli_query($conn, $sql);
    $num = mysqli_num_rows($result);
    if ($num == 1){
        while($row=mysqli_fetch_assoc($result)){
            if (password_verify($password, $row['compPass'])){ 
              echo 'successful';
                $login = true;
                session_start();
...
.c-received {
  ...
  <div class="container">
  <form action="EmpLog.php" method="post">
    <?php
    if($login){
    echo '<div class="c-received">
        <p class="c-receivepara">Logged in successfully </p>
      </div>';
    }
    if($showError){
      echo '<div class="c-received">
